...
def build_projects_report(request):
    ...
    if job_no: pri_allocs = pri_allocs.filter(job_no=job_no)
    if job_prefix: pri_allocs = pri_allocs.filter(job_no__startswith=job_prefix)
    if created_gte:
    ...
    line_allocs = PurchaseOrderLineAllocation.objects.select_related(
        "po_line__po__pr", "po_line__po__supplier",
        "po_line__purchase_request_item__item"
    )
    if job_no: line_allocs = line_allocs.filter(job_no=job_no)
    if job_prefix: line_allocs = line_allocs.filter(job_no__startswith=job_prefix)
    if created_gte:
        line_allocs = line_allocs.filter(Q(po_line__po__created_at__gte=created_gte) | Q(po_line__po__ordered_at__gte=created_gte))
    if created_lte:
        line_allocs = line_allocs.filter(Q(po_line__po__created_at__lte=created_lte) | Q(po_line__po__ordered_at__lte=created_lte))

    # Group by PO to apportion payment schedules later
    po_seen_by_job = defaultdict(set)  # job -> {po_id}
    po_info = {}  # po_id -> (currency, pr_rates, tax_rate, status, ts, supplier_name, total_amount)

    # First pass: commit & ordered qty
    for la in line_allocs:
        ...
        # tax on this allocation = PO tax_rate * allocation net
        tax_rate = po.tax_rate or Decimal("0")
        row["committed_tax_eur"] += (net_eur * (tax_rate/Decimal("100")))

        # ordered quantities by unit (from PR item’s unit)
        ...
        if po.id not in po_seen_by_job[j]:
            ...
            row["total_pos"] += 1
            if po.status == "cancelled": row["cancelled_pos"] += 1
            else: row["active_pos"] += 1

        po_info.setdefault(po.id, (po.currency or "TRY", pr_rates, po.tax_rate or Decimal("0"), po.status, ts, po.supplier.name if po.supplier else "", po.total_amount or Decimal("0.00")))

    # --- Apportion paid/unpaid schedules by allocation share ---
    from ..models import PaymentSchedule
    for j, po_ids in po_seen_by_job.items():
        row = proj[j]
        for po_id in po_ids:
            cur, pr_rates, tax_rate, status, ts, sup_name, po_total = po_info[po_id]
            if po_total is None or po_total == 0:
                continue
            # total allocated amount to this job for this PO:
            allo_sum = sum(
                (la.amount or Decimal("0.00"))
                for la in line_allocs.filter(po_line__po_id=po_id, job_no=j)
            )
            share = (allo_sum / po_total) if po_total else Decimal("0")
            if share <= 0:
                continue
            schedules = PaymentSchedule.objects.filter(purchase_order_id=po_id)
            for sch in schedules:
                amt_eur = to_eur(sch.amount, sch.currency or cur, pr_rates, fb)
                if amt_eur is None:
                    continue
                apportioned = amt_eur * share
                if sch.is_paid:
                    row["paid_eur"] += apportioned
                else:
                    row["unpaid_eur"] += apportioned

    # --- Pending PR estimate (recommended ItemOffer totals, split by PR allocations) ---
    # PR items that have no PO line yet -> estimate from recommended offer
    # (lightweight: we don’t handle partial-PO yet; refine later if needed)
    from ..models import SupplierOffer
    rec_items = (
        ItemOffer.objects
        .select_related("supplier_offer__purchase_request", "purchase_request_item__item", "supplier_offer")
    ...
    # Map PR item -> its allocations for share
    ...
    for io in rec_items:
        ...
        if total_eur is None:
            continue
        # split by allocation quantity share for this PR item
        allocs = pri_all_by_item.get(io.purchase_request_item_id, [])
        qty_sum = sum((a.quantity or Decimal("0")) for a in allocs) or Decimal("0")
        if qty_sum <= 0:
            continue
        for a in allocs:
            share = (a.quantity or Decimal("0")) / qty_sum
            row = proj.setdefault(a.job_no, {
                "job_no": a.job_no,
                "requested_qty_by_unit": defaultdict(Decimal),
                "ordered_qty_by_unit": defaultdict(Decimal),
                "distinct_items": set(),
    ...
    for j, r in proj.items():
        ...
        committed_gross = r["committed_net_eur"] + r["committed_tax_eur"]
        outstanding = (r["paid_eur"] + r["unpaid_eur"]) and (committed_gross - r["paid_eur"]) or (committed_gross)
        forecast = outstanding + r["pending_pr_estimate_eur"]

        is_empty = (
            active_pos == 0
            and committed_gross == 0
        ...
        rows.append({
            ...
            "committed_tax_eur": str(q2(r["committed_tax_eur"])),
            "committed_gross_eur": str(q2(committed_gross)),
            "paid_eur": str(q2(r["paid_eur"])),
            "unpaid_eur": str(q2(r["unpaid_eur"])),
            "outstanding_commitment_eur": str(q2(outstanding)),
            "pending_pr_estimate_eur": str(q2(r["pending_pr_estimate_eur"])),
            "forecast_eur": str(q2(forecast)),
            "top_suppliers_by_spend": [
                {"name": name, "total_eur": str(q2(val))}
                for name, val in r["top_suppliers"].most_common(5)
            ],
    ...
    if ordering:
        ...
        for k in reversed(keys):
            rev = k.startswith("-"); kk = k.lstrip("-")
            def keyer(row):
                if kk in {"forecast_eur","committed_gross_eur","pending_pr_estimate_eur","outstanding_commitment_eur"}:
                    return float(row[kk])
