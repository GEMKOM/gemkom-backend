...
def add_sales_order_confirmed_notify_config(apps, schema_editor):
    ...
    NotificationConfig.objects.filter(
        notification_type='sales_order_confirmed',
    ).update(default_send_in_app=False)


def remove_sales_order_confirmed_notify_config(apps, schema_editor):
    NotificationConfig = apps.get_model('notifications', 'NotificationConfig')
    NotificationConfig.objects.filter(notification_type='sales_order_confirmed_notify').delete()
    NotificationConfig.objects.filter(
        notification_type='sales_order_confirmed',
    ).update(default_send_in_app=True)


class Migration(migrations.Migration):

    dependencies = [
        ('notifications', '0032_alter_notification_notification_type_and_more'),
    ]
    ...
    operations = [
        ...
        migrations.RunPython(
            add_sales_order_confirmed_notify_config,
            reverse_code=remove_sales_order_confirmed_notify_config,
        ),
    ]
