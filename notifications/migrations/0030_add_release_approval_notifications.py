...
CHOICES = [
    ...
    ('release_approval_requested', 'Çizim Yayını Onay Bekliyor'),
    ('release_approved', 'Çizim Yayını Onaylandı'),
    ('release_rejected', 'Çizim Yayını Reddedildi'),
...
RELEASE_APPROVAL_CONFIGS = [
    {
        ...
        'link_template': f'{BASE_URL}/design/release-approvals/?release_id={{release_id}}',
        'available_vars': [
            'actor', 'job_no', 'job_title', 'revision', 'folder_path',
            'changelog', 'topic_id', 'release_id', 'link',
        ],
    },
    {
        ...
        'link_template': f'{BASE_URL}/design/release-approvals/?release_id={{release_id}}',
        'available_vars': [
            'actor', 'job_no', 'job_title', 'revision', 'topic_id', 'release_id', 'link',
        ],
    },
    {
        ...
        'link_template': f'{BASE_URL}/design/projects/?task={{task_id}}',
        'available_vars': [
            'actor', 'job_no', 'job_title', 'revision', 'reason',
            'topic_id', 'release_id', 'task_id', 'link',
        ],
    },
]


def add_release_approval_configs(apps, schema_editor):
    NotificationConfig = apps.get_model('notifications', 'NotificationConfig')
    for cfg in RELEASE_APPROVAL_CONFIGS:
...
def remove_release_approval_configs(apps, schema_editor):
    NotificationConfig = apps.get_model('notifications', 'NotificationConfig')
    NotificationConfig.objects.filter(
        notification_type__in=[c['notification_type'] for c in RELEASE_APPROVAL_CONFIGS]
    ).delete()
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('notifications', '0029_add_vacation_cancellation_notifications'),
    ]
    ...
    operations = [
        ...
        migrations.RunPython(
            add_release_approval_configs,
            reverse_code=remove_release_approval_configs,
        ),
    ]
